...
must_haves:
  ...
  key_links:
    ...
    - from: "src/workers/workflow.worker.ts"
      to: "src/services/notification/notification-dispatcher.service.ts"
      via: "Send failure notifications to creator"
      pattern: "notificationDispatcher\\.send"
---
...
1. Create processWorkflowJob function:
   - Load execution from database with workflow relation:
     ```typescript
     const execution = await prisma.workflowExecution.findUnique({
       where: { id: job.data.executionId },
       include: {
         workflow: {
           include: { createdBy: true }  // Load creator for notifications
         }
       }
     });
     ```
...
2. Handle execution failures (per user decision):
   On failure, send notifications to three recipients using notificationDispatcher:

   a. Notify incident assignee:
      - Load incident with assignee relation
      - Call notificationDispatcher.send(assignee, failureMessage)

   b. Notify workflow creator:
      - Extract creator from execution.workflow.createdBy (loaded above)
      - Get creator's email from createdBy.email
      - Call notificationDispatcher.send(creator, failureMessage)

   c. Notify team channel (Slack/Teams):
      - Load team settings for workflow's teamId
      - Call notificationDispatcher.sendToChannel(teamChannel, failureMessage)
...
Worker should handle errors gracefully and never crash the process.
  ...
  <done>BullMQ queue and worker process workflows with failure notifications to assignee, creator, and team channel</done>
</task>
