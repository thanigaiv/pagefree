...
<task type="auto">
  <name>Task 3: Integrate Socket.io with Express and Incident Service</name>
  <files>
    src/index.ts
    src/services/incident.service.ts
  </files>
...
Also update the routing service (where incidents are created) to broadcast incident:created events. In src/services/routing.service.ts or wherever createIncident is called, add after incident creation:
...
grep -q "socketService" src/services/incident.service.ts
  ...
  <done>
Socket.io integrated with Express HTTP server. Incident service broadcasts events on acknowledge, resolve, reassign, and note added. Server starts with WebSocket support.
  </done>
</task>
