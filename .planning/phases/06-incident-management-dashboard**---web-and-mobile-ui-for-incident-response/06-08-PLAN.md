...
must_haves:
  truths:
    ...
    - "PWA install prompt appears after first acknowledgment"
  artifacts:
  ...
  key_links:
    ...
    - from: "frontend/src/components/IncidentRow.tsx"
      to: "frontend/src/hooks/usePWA.ts"
      via: "promptAfterAcknowledge callback"
      pattern: "promptAfterAcknowledge"
---
...
<task type="auto">
  <name>Task 3: Update IncidentRow for Swipe with PWA Install Prompt</name>
  <files>
    frontend/src/components/MobileLayout.tsx
    frontend/src/pages/SchedulePage.tsx
    frontend/src/pages/ProfilePage.tsx
...
Update frontend/src/components/IncidentRow.tsx to use SwipeableRow on mobile with PWA install prompt:
...
Wrap the Card component with SwipeableRow and wire PWA prompt per locked decision:
```typescript
...
grep -q "promptAfterAcknowledge" frontend/src/components/IncidentRow.tsx
cd frontend && npm run build
  </verify>
  <done>
MobileLayout wraps app with bottom navigation. Schedule and Profile placeholder pages created. IncidentRow uses SwipeableRow for mobile swipe gestures. PWA install prompt triggered after first acknowledgment per locked decision. All routes configured.
  </done>
</task>
...
9. After first swipe-acknowledge, verify PWA install prompt appears (if not already installed)
</verification>
