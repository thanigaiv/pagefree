...
must_haves:
  truths:
    ...
    - "Backend endpoint exists to create notes"
  artifacts:
    ...
    - path: "src/routes/incident.routes.ts"
      provides: "POST /incidents/:id/notes endpoint"
      contains: "addNote"
  key_links:
    ...
    - from: "src/routes/incident.routes.ts"
      ...
      via: "incidentService.addNote"
      pattern: "incidentService\\.addNote"
---
...
Output: Markdown editor with preview tabs, inline note submission, and backend endpoint.
</objective>
...
<task type="auto">
  <name>Task 2: Create Note Hook, AddNoteForm, and Backend Endpoint</name>
  <files>
    frontend/src/hooks/useAddNote.ts
    frontend/src/components/AddNoteForm.tsx
    src/routes/incident.routes.ts
...
Add backend endpoint in src/routes/incident.routes.ts:

Add the following route after the existing incident routes (e.g., after acknowledge, resolve routes):
```typescript
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Call incident service to add note (this method should already exist from Phase 2)
      await incidentService.addNote(id, userId, note.trim());
...
If incidentService.addNote doesn't exist, add it to src/services/incident.service.ts:
```typescript
async addNote(incidentId: string, userId: string, note: string): Promise<void> {
...
grep -q "/:id/notes" src/routes/incident.routes.ts
cd frontend && npm run build
npm run build
  </verify>
  <done>
Note mutation hook and inline AddNoteForm with collapsible expand. Backend POST /incidents/:id/notes endpoint added. Form clears and closes on successful submission. WebSocket broadcast on note added.
  </done>
</task>
...
2. `npm run build` - Backend compiles without errors
3. Open incident detail view
4. Click "Add Note" - verify form expands inline
5. Type markdown (e.g., **bold**, `code`)
6. Switch to Preview tab - verify markdown renders
7. Submit note - verify it appears in timeline
8. Test metadata editor - edit and save values
9. Test backend endpoint: `curl -X POST /api/incidents/:id/notes -d '{"note": "test"}'`
</verification>
...
- [ ] Backend POST /incidents/:id/notes endpoint exists and works
</success_criteria>
